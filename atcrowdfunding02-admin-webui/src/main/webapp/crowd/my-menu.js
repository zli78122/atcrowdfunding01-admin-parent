	...
	$.ajax({
		...
		"success":function(response){
			...
			if(result == "SUCCESS") {
				...
				var setting = {
					"view": {
						"addDiyDom": myAddDiyDom,			// 修改默认的图标
						"addHoverDom": myAddHoverDom,		// 在鼠标移入节点范围时添加按钮组
						"removeHoverDom": myRemoveHoverDom	// 在鼠标离开节点范围时删除按钮组
					},
					"data": {
						"key": {
							"url": "disableJump" // 设置url属性值为一个不存在的值，可以实现点击树形节点不跳转页面
						}
					}
				};
...
function myAddDiyDom(treeId, treeNode) {
	...
	// treeNode是当前树形节点的全部数据，包括从后端查询得到的Menu对象的全部属性
...
function myAddHoverDom(treeId, treeNode) {
	...
	// 为了在需要移除按钮组的时候能够精确定位到按钮组所在span，需要给span设置一个有规律的id
